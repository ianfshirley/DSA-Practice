...
const count = (target, input) => {
  let count = 0;
  input.forEach(arr => arr.forEach((num) => {
    if (num === target) {
      count += 1;
    }
  }));
...
const totalSum = (input) => {
  return input.reduce((acc, row) => {
    let rowSum = row.reduce((rowAcc, num) => {
      rowAcc += num;
      return rowAcc;
    }, 0);
    acc += rowSum;
...
const divisibleByFiveTwoToThePower = (input) => {
  let div5 = input.map(arr => arr.filter(num => num % 5 === 0 && typeof num === 'number'));
  return div5.map(arr => arr.map(num => Math.pow(2,num)));
};
...
let findShortest = (data) => {
  ...
  //   return parseInt(curr.height) < parseInt(shortest.height) ? curr : shortest;
  // }).name;
